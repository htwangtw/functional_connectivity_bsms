...
def get_path(subject_id, i):
    nii_path = (
        base_path
        / f"{subject_id}/MNINonLinear/Results/rfMRI_REST{i}_AP/rfMRI_REST{i}_AP_hp2000_clean.nii.gz"
    )
    if nii_path.exists():
...
participants = pd.read_csv(
    project_path / "sourcedata/participant_info.csv", skipinitialspace=True
)
...
participants = participants.rename(
    columns={"CISC": "subject_label", "Group": "patient"}
)
